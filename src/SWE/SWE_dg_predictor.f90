...
!             volume_flux(j,:,:) = &
!                  matmul(jacobian_inv(1,1) * matmul(s_m_inv,transpose(s_k_x)), f_ref(1,j,:,:)) +&
!                  matmul(jacobian_inv(1,2) * matmul(s_m_inv,transpose(s_k_x)), f_ref(2,j,:,:)) +&
!                  matmul(jacobian_inv(2,1) * matmul(s_m_inv,transpose(s_k_y)), f_ref(1,j,:,:)) +&
!                  matmul(jacobian_inv(2,2) * matmul(s_m_inv,transpose(s_k_y)), f_ref(2,j,:,:))
!          end do
...
!          ! do j=1,_SWE_DG_ORDER+1
!          !    volume_flux(j,:,:) = matmul(matmul(s_m_inv,transpose(s_k_x)),f(1,j,:,:)) + matmul(matmul(s_m_inv,transpose(s_k_y)),f(2,j,:,:))
!          ! end do

!          do i=1,_SWE_DG_DOFS
!             q_temp_st(:,i,:) =  -matmul(matmul(t_k_t_11_inv,t_m_1), volume_flux(:,i,:))
!          end do
...
!          do i=1,_SWE_DG_DOFS         
!             q_temp_st(:,i,:) = q_temp_st(:,i,:) + matmul(t_k_t_11_inv,source_st_red(:,i,:))
!          end do
