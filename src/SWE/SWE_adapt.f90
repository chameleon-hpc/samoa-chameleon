          ...
          if(.not.isCoast(src_element%cell%data_pers%troubled)) then
             associate(src => src_element%cell%data_pers, dest => dest_element%cell%data_pers)
               do i=1, size(refinement_path)
                  ...
                  if ( (refinement_path(i) == 1 .and. i_plotter_type>0) .or.&
                       (refinement_path(i) == 2 .and. i_plotter_type<0)) then
                     dest%Q%H    = matmul(ref2,src%Q%H+src%Q%B)
                     dest%Q%p(1) = matmul(ref2,src%Q%p(1))
                     dest%Q%p(2) = matmul(ref2,src%Q%p(2))
                  else
                     dest%Q%H    = matmul(ref1,src%Q%H+src%Q%B)
                     dest%Q%p(1) = matmul(ref1,src%Q%p(1))
                     dest%Q%p(2) = matmul(ref1,src%Q%p(2))
                  end if

                dest%Q%H   = matmul(s_m_inv,dest%Q%H)
                dest%Q%p(1)= matmul(s_m_inv,dest%Q%p(1))
                dest%Q%p(2)= matmul(s_m_inv,dest%Q%p(2))
