...
if(data%troubled.eq.2) then
   call apply_phi(data%Q_DG(:)%h,data%H)
...
#                       endif
              ...
              section%r_dt_new = min(section%r_dt_new, volume / (edge_lengths(2) * maxWaveSpeed) )
               ...
               call bathymetry_derivatives(element%cell%data_pers,ref_plotter_data(element%cell%geometry%i_plotter_type)%jacobian_normalized)
