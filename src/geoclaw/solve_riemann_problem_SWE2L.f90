subroutine solve_riemann_problem_SWE2L(meqn,mwaves, n_index,t_index, h_l,h_r, hu_l,hu_r, hv_l,hv_r, b_l,b_r, h_hat_l,h_hat_r, fw_priv, sw_priv, dry_tolerance, g)
    ...
    if (( (h_l(1) + h_l(2) + b_l == h_r(1) + h_r(2) + b_r .or. (dry_state_l(2) .and. dry_state_r(2))) .or. &                  ! IF (first layer dry OR at constant level OR 
          (dry_state_l(1) .and. h_r(1) + h_r(2) + b_r < b_l) .or. (dry_state_r(1) .and. h_l(1) + h_l(2) + b_l < b_r)) .and. & ! first layer consists of a sea wall) AND   
        ( (h_l(2) + b_l == h_r(2) + b_r .or. (dry_state_l(2) .and. dry_state_r(2))) .or. &                                    ! (second layer dry OR at constant level OR
          (dry_state_l(2) .and. h_r(2) + b_r < b_l) .or. (dry_state_r(2) .and. h_l(2) + b_l < b_r) ) .and. &                  ! second layer is a sea wall) AND
        hu_l(1) == 0.0d0 .and. hu_l(2) == 0.0d0 .and. hv_l(1) == 0.0d0 .and. hv_l(2) == 0.0d0 .and. &                         ! momentum=zero in left cell AND
        hu_r(1) == 0.0d0 .and. hu_r(2) == 0.0d0 .and. hv_r(1) == 0.0d0 .and. hv_r(2) == 0.0d0  ) then                         ! momentum=zero in right cell THEN solution <- zero
...
!                      .not. dry_state_r(1) .and. .not. dry_state_r(2)) then
    ...
    ! HACK: apply a limiter to avoid insignificant waves that don't contribute much to the actual solution
    ! but end up affecting the cell refinement when they reach the shoreline
    where (abs(fw_priv) < 1e-10)
        fw_priv = 0.0d0
    end where
