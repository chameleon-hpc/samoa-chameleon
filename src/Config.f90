...
#       endif
    ...
    subroutine define_arguments(config, arguments)
        ...
        write(arguments, '(A, A)') trim(arguments),   " -lbhh .false. -lbfreq 1 -lbhhthreshold 0.1 -lbhhauto .false. -lbhhratio 1 1 "
...
#       endif
        ...
        if (rank_MPI == 0) then
            ...
            if (l_help) then
                ...
                PRINT '(A, L, A)',      "   -lbhh                   if true, MPI load balancing can distribute the load unevenly (value: ", config%l_lb_hh, ")"
                PRINT '(A, A, A)',      "   -lbhhratio              if using heterog. LB, first choose a fixed Host/MIC ratio (value: ", trim(config%s_lb_hh_ratio), ")"
...
#       endif
        ...
        if (config%l_lb_hh_auto) then
            _log_write(0, '(" Load balancing: for heterogenous hardware (HH): ", A, ", threshold: ", F0.3, ", ratio: ", A)') logical_to_char(config%l_lb_hh), config%r_lb_hh_threshold, "auto"
        else
            _log_write(0, '(" Load balancing: for heterogenous hardware (HH): ", A, ", threshold: ", F0.3, ", ratio: ", F0.1, "/", F0.1)') logical_to_char(config%l_lb_hh), config%r_lb_hh_threshold, config%r_lb_hh_ratio(1), config%r_lb_hh_ratio(2)
        end if
