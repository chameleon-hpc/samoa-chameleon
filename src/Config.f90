...
#       endif
        ...
        if (config%l_lb_hh_auto) then
            ...
            _log_write(0, '(" Load balancing: for heterogenous hardware (HH): ", A, ", ratio: ", F0.1, "/", F0.1)') logical_to_char(config%l_lb_hh), config%r_lb_hh_ratio(1), config%r_lb_hh_ratio(2)
        end if
